...
func TestCancelUpdateProject(t *testing.T) {
	var (
	...
	awx := NewAWX(testAwxHost, testAwxUserName, testAwxPasswd, nil)
	result, err := awx.ProjectService.CancelUpdate(80)
	if err != nil {
...
func TestGetUpdateProject(t *testing.T) {
	var (
	...
	awx := NewAWX(testAwxHost, testAwxUserName, testAwxPasswd, nil)
	result, err := awx.ProjectService.GetUpdate(301)
	if err != nil {
		t.Fatalf("GetUpdateProject err: %s", err)
	} else {
		checkAPICallResult(t, expectProjectUpdateGetResponse, result)
		t.Log("GetUpdateProject passed!")
	}
}
